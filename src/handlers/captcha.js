...
async function captchaLevel2(channel, botId, embed) {
  console.warn(
    "[captchaLevel2] ⚠  Level 2 Captcha detected (stub) — awaiting implementation.",
  );
}
...
async function captchaLevel1(channel, botId, embed) {
  ...
  if (!code) {
    console.warn(
      "[captchaLevel1] No code could be extracted from the embed. Triggering Level 2 Captcha...",
    );
    await captchaLevel2(channel, botId, embed);
    return;
  }

  console.log(`[captchaLevel1] 🔐 Captcha code extracted: "${code}"`);
  console.log(
    `[captchaLevel1] Sending Slash Command: "/verify answer:${code}"...`,
  );

  try {
    await channel.sendSlash(botId, "verify", code);
    console.log("[captchaLevel1] ✅ Verify command sent successfully!");
  } catch (err) {
    console.error(
      "[captchaLevel1] ❌ Error during verification flow:",
      err.message,
    );
  }
}
